...
if (isset($_POST['boton'])) {  $nombre = $_FILES['archivo']['name'];
  ...
  $destino = "archivos/" . $nombre;
      if( $nombre != ""){		 if (copy($ruta, $destino)){			  $ape= $_POST['ape'];            $nom= $_POST['nom'];            $dni= $_POST['dni'];           $distri= $_POST['distri'];           $date= $_POST['date'];           $serv= $_POST['serv'];           $proyect= $_POST['proyect'];		   		   $db= new Conect_Mysql();		   		     $sql= "INSERT INTO documentos('',Apellido, Nombre, DNI, Distrito, Fecha Emitida, Tipo de Servicio, Tipo de Archivo, Nombre del archivo, Tipo, Tamaño) 	       VALUES ( '" .$ape."','".$nom."','".$dni."','".$distri."','".$date."','".$serv."','".$proyect."','".$nombre."','".$tipo."',".$tamanio."); ";						$query = $db->execute($sql);			if ($query){				echo 'Se guardo correctamente';			}			else {				echo 'Error';			}		 }		 	 }
   }
